...
# Uses powerline git branch icon  (U+E0A0) as leading prefix when in a git repo
__compact_path() {
    local git_root
    ...
    if [[ -n "$git_root" ]]; then
        ...
        # Powerline git branch icon: U+E0A0 (UTF-8 bytes: EE 82 A0)
        local git_icon=$'\xee\x82\xa0'
